        ...
        for project in projectStage['items']:
            with urllib.request.urlopen(landscapeSingleItem.format(project['id'])) as singleItemResponse:
                ...
                csvRows.append({
                        ...
                        'Leads': projectData['extra']['leads'] if 'leads' in projectData['extra'] else None,
                        'Meeting Cadence': projectData['extra']['meeting_cadence'] if 'meeting_cadence' in projectData['extra'] else None,
                        'LFX Insights URL': projectData['extra']['lfx_insights_url'] if 'lfx_insights_url' in projectData['extra'] else None,
                        'Accepted Date': projectData['extra']['date_accepted'] if 'date_accepted' in projectData['extra'] else None,
                        'Last Review Date': projectData['extra']['last_review_date'] if 'last_review_date' in projectData['extra'] else None,
                        'Next Review Date': projectData['extra']['next_review_date'] if 'next_review_date' in projectData['extra'] else None,
                        'Slack': projectData['extra']['slack_channel'] if 'slack_channel' in projectData['extra'] else None,
